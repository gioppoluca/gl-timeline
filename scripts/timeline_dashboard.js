...
export default class TimelineDashboard extends HandlebarsApplicationMixin(ApplicationV2) {
  constructor(options = {}) {
    super(options);
  ...
  static DEFAULT_OPTIONS = {
    id: "gl-timetable",
    form: {
      handler: TimelineDashboard.myFormHandler,
      closeOnSubmit: true,
    },
    position: {
      width: 640,
  ...
  static PARTS = {
    form: {
      template: "modules/gl-timeline/scripts/templates/timeline_dashboard.html"
    }
  }
  ...
  async openLink(link) {
    ...
    const page = await fromUuid(link);
